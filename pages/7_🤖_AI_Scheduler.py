...
# Auto-load jika data belum siap
if not is_data_ready():
    if not load_data_from_database():
        load_data_from_files()

# --- TAMPILAN UTAMA HASIL & LAPORAN ---
if is_data_ready():
    ...
    if st.button("🚀 Generate Jadwal & Laporan Guru"):
        ...
        if not df_hasil.empty:
            ...
            with tab2:
                ...
                st.caption("Menampilkan status harian, jumlah JP, kelas diampu, serta jam kosong sela.")

                # Filter Pilihan Guru
                if "ID_Guru" in df_laporan_guru.columns:
            ...
            st.error("❌ Solver tidak dapat menemukan kombinasi jadwal yang cocok. Naikkan batas Timeout.")

else:
    st.warning("⚠️ Data Master belum lengkap di memori. Unggah file Excel master (yang berisi sheet Guru, Rombel, Mengajar, Mapel, Slot) di bawah ini:")
