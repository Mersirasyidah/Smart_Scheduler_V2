...
class Scheduler:
    ...
    def _solve_skenario(self, timeout_sec, strict_mgmp=True):
        ...
        # 1. Constraint JP Terpenuhi Sesuai Beban_JP di Sheet Mengajar
        for t in tugas_info:
            model.Add(
        ...
        # 3. Guru Tidak Bentrok Mengajar di 2 Tempat
        ...
        # 4. MAX 2 JP PER HARI PER MAPEL DI KELAS YANG SAMA
        for r in rombel_list:
            mapel_in_rombel = set(
        ...
        # 5. ATURAN KHUSUS MAPEL M08 (Jam 1 - 4)
        for t in tugas_info:
        ...
        if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
            ...
            # Hitung Laporan Beban Guru secara Akurat (Total JP Terjadwal per Guru)
            df_laporan = (
    ...
    def solve_with_fallback(self, timeout_total=180, progress_callback=None):
        if progress_callback:
            progress_callback(
                "Mencari jadwal optimal (Max 2 JP/hari & M08 di jam 1-4)..."
            )
